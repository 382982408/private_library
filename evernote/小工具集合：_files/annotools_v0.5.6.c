...
////gcc -lz -std=gnu99 annotools_v0.5.5.c 
...
/////////////////////////////////
/*
void vcfInfo_standardized (char ori[]) //ori can not change
{
	while (1)
	{
		if (strstr(ori,";.;"))
...
int get_annovcf(char i[],char o[],char d[], char n[])
{
    ...
    while(fgets(LINE_IN,sizeof(LINE_IN),IN)!=NULL)
    {
        ...
        if(bcmp(LINE_IN,"#",1)==0)
        {
			if (strstr(LINE_IN,"CHROM")!=NULL)
			{
				sprintf(CHROM_header,"%s",LINE_IN);	
			}else
			{
				fputs(LINE_IN,OUT);
			};
...
//          fputs(LINE_IN,OUT);
		...
		if (chr < buff_chr)
		{
			fputs(LINE_IN,OUT);
			continue;
		}else if(chr == buff_chr && start < buff_start)
		{
			fputs(LINE_IN,OUT);
			continue;
		}else if(chr == buff_chr && start == buff_start && strcmp(ref,buff_ref)<0)
		{
			fputs(LINE_IN,OUT);
			continue;
		}else if(chr == buff_chr && start == buff_start && strcmp(ref,buff_ref)==0 && strcmp(alt,buff_alt)<0)
		{
			fputs(LINE_IN,OUT);
			continue;
		};
        ...
        while (idx_tag==1 && ((chr > idx_chr) || (chr==idx_chr && start > idx_end)))
        {
            ...
            //fseek(ON,seek_down,SEEK_SET);
			if (seek_down > seek_before)
			{
				seek_before=seek_down-idx_diff;
				seek_after=seek_before;
			};
        };
        ///////////////
/*
        float seek_true=ftell(ON);
        seek_true=seek_true-(seek_after-seek_before+10);
        if(seek_true<0)
        {
            seek_true=0;
        };
*/		
		if(CHROM_tag > 0)
		{
			if ((seek_before-idx_diff)<0)
			{
				fseek(ON,0,SEEK_SET);
			}else
			{
				fseek(ON,(seek_before-idx_diff),SEEK_SET);
			};
		};
        //////////////////////
        int report_tag=0;
        ...
        while(fgets(LINE_ON,sizeof(LINE_ON),ON)!=NULL)
        {
			...
			buff_chr=chr_2;
			buff_start=start_2;
			sscanf(ref_2,"%s",buff_ref);
			sscanf(alt_2,"%s",buff_alt);
            //////////////////////
            //printf("CCC\t%s\t%f\t%d\t%d\t%d\t%s\n",LINE_ON,seek_down,chr_2,start_2,end_2,com_2);
                ...
                }else
                {
                    ...
                    }else
                    {
                         ...
                         {
							 LINE_IN[strlen(INFO)-1]=';';
							 if (bcmp(n,"-",1)==0)
							 {
								 char last_info[50000];
								 sprintf(last_info,"%s;%s",INFO,info);
								 strrpc(last_info,";.;",";");
								 strrpc(last_info,";;",";");
								 if (last_info[strlen(last_info)-1] == ';')
								 {
									last_info[strlen(last_info)-1]='\0';
								 };
								 
								 sprintf(report,"%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s%s\n",rubish_1,start,rs,ref,alt,qua,filter,last_info,other);
								 //printf("%s\n",report);
								 //exit(1);
								 fputs(report,OUT);
								 report_tag=1;
							 ...
							 }else
							 {
								 ...
								 char last_info[50000];
								 sprintf(last_info,"%s;%s",INFO,last);
								 strrpc(last_info,";.;",";");
								 strrpc(last_info,";;",";");
								 if (last_info[strlen(last_info)-1] == ';')
								 {
...
//////////////////////////////////////////////////////
//////////////////////////////////////////////////////
int get_annovcfgz(char i[],char o[],char d[], char n[])
{
    printf("annovcf start...\n\n");
    ////////
    long idx_chr=1;
	...
	while(gzgets(IN,LINE_IN,sizeof(LINE_IN))) 
    //while(fgets(LINE_IN,sizeof(LINE_IN),IN)!=NULL)
    {
		//printf("AAA\t%s%f\n",LINE_IN,seek_before);
        if(bcmp(LINE_IN,"#",1)==0)
...
int help(char prog[],char cmd[],char i[],char o[],char d[],char n[])
{
    ...
    }else if(strlen(cmd)>0 && strcmp(cmd,"annovcf")==0)
    {
        ...
        }else
        {
			if (strstr(i,".gz"))
			{
				get_annovcfgz(i,o,d,n);
			}else if (strstr(i,".vcf"))
			{
				get_annovcf(i,o,d,n);
			}else
			{
				printf("\terror:input file must be .vcf or .vcf.gz, please check your input file !\n\n");
				exit(1);
			};
            return 0;
        };		
